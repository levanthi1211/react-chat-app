...
export const RequestContact: FC<RequestContactProps> = (props) => {
  const [mountedContact, setMountedContact] = useState<any>(false);
  const { onClose } = props;
  const afterSubmit = () => {
    onClose();
  };

  const { data, methods } = useRequestContact({ afterSubmit });
  const dispatch = useAppDispatch();

  const handleRequestContact = async (e: FormEvent) => {
    e.preventDefault();
    await methods.handleSubmitRequestContact();
    dispatch(getProfileInformationAction());
  };

  const handleSearchContact = async (e: ChangeEvent<HTMLInputElement>) => {
    await methods.handleSearchContact(e.target.value);
  };

  const { foundContacts, dirtyFields } = data;
  ...
  return (
    <div className="fixed top-0 left-0 w-screen h-screen bg-black/50 flex justify-center items-center">
      <div className="w-[500px] bg-white rounded">
        ...
        <form onSubmit={handleRequestContact} autoComplete="off">
          <div className="p-6">
            {!mountedContact && (
              <input
                id="email1"
                ...
                {...data.register("email", {
                  required: true,
                  onChange: _.debounce(handleSearchContact, 300),
                })}
              />
            )}
            {mountedContact && (
              <div className="flex justify-between border border-primary transition-all duration-100 items-center cursor-pointer rounded p-2">
                <div className="flex justify-start items-center gap-2 my-2">
                  <div
                    ...
                    style={{
                      backgroundImage: `url(${
                        mountedContact.avatar_url
                          ? getAvatar(mountedContact.avatar_url)
                          : avatarPlaceholder
                      })`,
                    }}
                  >
                  ...
                  <div>
                    <span className="text-[#a188a8] text-sm font-bold noselect">
                ...
                <div onClick={() => setMountedContact(false)}>
            ...
            {foundContacts.length > 0 && !mountedContact && (
              <>
                <h1 className="inline-block py-2 font-bold text-primary">
                  Result:
                </h1>
                {foundContacts.map((foundContact: any, index: number) => {
                  return (
                    <div
                      key={index}
                      className={clsx(
                        "flex justify-between border border-primary transition-all duration-100 items-center cursor-pointer rounded p-2 mb-4",
                        [foundContact.isSent ? "" : "hover:bg-primary/30"]
                      )}
                      onClick={() => {
                        if (!foundContact.isSent) {
                          setMountedContact(foundContact);
                          methods.setValue("email", foundContact.email);
                        }
                      }}
                    >
                      <div className="flex justify-start items-center gap-2 my-2">
                          ...
                          style={{
                            backgroundImage: `url(${
                              foundContact.avatar_url
                                ? getAvatar(foundContact.avatar_url)
                                : avatarPlaceholder
                            })`,
                          }}
                        ></div>
                      ...
                      {foundContact.isSent && (
                        <div>
                          <button
                            className={clsx(
                              "cursor-pointer bg-primary/20 px-2 h-8 flex justify-center items-center",
                              [foundContact.isSent && "hover:bg-primary group"]
                            )}
                            onClick={() => {
                              console.log("a");
                            }}
                          >
                            <span
                              className={clsx("text-primary", [
                                foundContact.isSent && "group-hover:text-white",
                              ])}
                            >
                              Accept request
                            </span>
          ...
          <div className="p-6">
            <textarea
              className="w-full bg-[#f6f6f9] border-0 rounded text-[13px] font-thin resize-none"
              rows={4}
              placeholder="Enter message here..."
              {...data.register("invitationMessage", {
                required: true,
              })}
            />
          </div>
