...
import { useChangeMail } from "./useChangeMail";

interface EditNameProps {
  onClose: any;
}

export const ChangeMail: FC<EditNameProps> = (props) => {
  const { onClose } = props;
  const afterSubmit = () => {
    onClose();
  };

  const { data, methods } = useChangeMail({ afterSubmit });
  const dispatch = useAppDispatch();

  const handleEditName = async (e: FormEvent) => {
    e.preventDefault();
    await methods.handleSubmitChangeEmail();
      ...
      <div className="w-[500px] bg-white rounded">
        ...
        <form onSubmit={handleEditName}>
          <div className="p-6">
            ...
            />
            {data.formError.email && (
              <span role="alert">
                {data.formError.email.message?.toString()}
              </span>
            )}
          </div>
